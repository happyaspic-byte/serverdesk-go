...
type DisplayMeta = poller.DisplayMeta
...
func metaNodes(view map[string]any) []any {
	...
	for _, nv := range listVal(view["nodes"]) {
		...
		if standing == "" {
			if lowerStr(strVal(n["state"])) == "running" {
				standing = "normal"
			} else {
				standing = "unknown"
			}
...
func metaTopo(view map[string]any) map[string]any {
	...
	for _, gv := range listVal(view["storage_groups"]) {
		...
		if p, ok := numVal(pct); ok {
			warn, crit := poller.UsageThresholds()
			if p >= crit {
				st = "down"
			} else if p >= warn {
				st = "deg"
